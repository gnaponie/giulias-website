...
export function VideoTagClient({
  tag,
  videosEn,
  videosIt,
}: {
  ...
  const s = t[lang];
  const videos = lang === "it" && videosIt.length > 0 ? videosIt : videosEn;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-16">
      ...
      {videos.length === 0 ? (
        <p className="text-muted-foreground">{s.noVideos}</p>
      ) : (
                ...
                <div className="mt-1 flex items-center gap-3 text-sm text-muted-foreground font-mono">
                  ...
                  {video.meta.date && (
                    <time className="tabular-nums">
                      {new Date(video.meta.date).toLocaleDateString(
                        lang === "it" ? "it-IT" : "en-US",
