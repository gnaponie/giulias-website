...
export function BlogClient({
  ...
  if (posts.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-16">
        <h1 className="font-mono text-2xl font-bold">{s.blog}</h1>
        <p className="mt-4 text-muted-foreground">{s.noPosts}</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16">
      <h1 className="font-mono text-2xl font-bold mb-8">{s.blog}</h1>
      <div className="flex flex-col sm:flex-row gap-10">
        ...
        {tags.length > 0 && (
          <aside className="sm:w-40 shrink-0 sm:border-l sm:border-border sm:pl-6">
            <h2 className="font-mono text-sm font-semibold text-muted-foreground mb-3">
              {s.tags}
            </h2>
